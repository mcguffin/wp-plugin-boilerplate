...
//		add_action('init',array( $this, 'init' ) );
		...
		add_option( '{{plugin_slug}}_cronjobs', array() );
		add_option( '{{plugin_slug}}_cronschedules', array() );
	...
	private function init() {
		$this->jobs = get_option( '{{plugin_slug}}_cronjobs' );
	...
	public function get_schedule( $interval ) {
		...
		if ( ! $schedule = $this->find_schedule( $interval ) ) {
			...
			update_option( '{{plugin_slug}}_cronschedules', $schedules );
		}
		return $schedule;
	}
	...
	public function cron_schedules( $schedules ) {
		return $schedules + get_option('{{plugin_slug}}_cronschedules');
	}
	...
	private function update() {
		...
		update_option( '{{plugin_slug}}_cronjobs', $this->jobs );
	...
	public function deactivate() {
		// stop all jobs
		$jobs = get_option( '{{plugin_slug}}_cronjobs', $this->jobs );
