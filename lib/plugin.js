...
const unwp = s => s.replace(/^(wp|wordpress)(\s|-|_)/i,'');
...
class Plugin {
	...
	async setup() {
		...
		let c = Object.values(this.components), cm;
		while ( c.length ) {
			await c.shift().setup();
		}
	...
	async generate() {
		let c = Object.values(this.components);
		while ( c.length ) {
	...
	guessProps() {
		...
		if ( this.prefix === false ) {
			this.prefix = change_case.snakeCase( this.short_name )
		}
	...
	get package() {
		return {
			name:		this.name, // WordPress Some PLugin
			slug:		this.slug, // wp-some-plugin
			prefix:		this.prefix, // some_plugin
			textdomain:	this.textdomain, // wp-some-plugin
			namespace:	this.namespace, // SomePlugin

			author:		this.author,
			author_uri:	this.author_uri,
			year:		this.year,
			components:	this.componentsPackage
		};
	}
	set package( p ) {
		Object.keys(p).filter( k => k !== 'components' ).forEach( k => this[k] = p[k] )
		if ( !! p.components ) {
			this.componentsPackage = p.components;
		}
	}

	get componentsPackage() {
		let c = {};
		Object.keys( this.components ).map( k => c[k] = this.components[k].package );
		return c;
	}
	set componentsPackage( c ) {
		Object.keys(c).forEach( k => this.addComponent( k, c[k] ) )
	}
	...
	addComponent( slug, p ) {
		if ( ! this.components[slug] ) {
			...
			_.merge(this.components[slug].package,p)
		}
